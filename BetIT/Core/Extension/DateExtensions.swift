...
public extension Date {
    ...
    func secondsSince(_ date: Date) -> Int {
        ...
        if let standardizedDate = Calendar.current.date(from: components) {
            return Int(timeIntervalSince(standardizedDate))
        }

        return Int(round(timeIntervalSince(date)))
    }
    ...
    func minutesSince(_ date: Date) -> Int {
        ...
        if let standardizedDate = Calendar.current.date(from: components) {
            return Int(timeIntervalSince(standardizedDate) / 60)
        }

        return Int(round(timeIntervalSince(date) / 60))
    }
    ...
    func daysSince(_ date: Date) -> Int {
        ...
        if let standardizedDate = Calendar.current.date(from: components) {
            return Int(timeIntervalSince(standardizedDate) / (3600 * 24))
        }
        return Int(round(timeIntervalSince(date) / ( 3600 * 24)))
    }
    
    /// SwifterSwift: get number of years between two date
    ///
    /// - Parameter date: date to compate self to.
...
//Extension for app
extension Date {
    static var standardDateFormat: String {
        return "yyyy/MM/dd"
    }
    
    static var standardTimeFormat: String {
        return "HH:mm:ss"
    }
    
    static var standardAMPMFormat: String {
        return "hh:mm a"
    }
    
    static var standardDateTimeFormat: String {
        return standardDateFormat + " " + standardTimeFormat
    }
    
    func weekdayString() -> String {
        let dateFormatter = DateFormatter()
        return dateFormatter.shortWeekdaySymbols[self.weekday - 1]
    }
    ...
    func homeBetCellString() -> String {
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = "hh:mm a\nMM/dd/yy"
        return dateFormatter.string(from: self)
    }
    
    func periodStringSince(_ date: Date = Date()) -> String {
        let seconds = Int(abs(self.secondsSince(date)))
    ...
    func longPeriodStringSince(_ date: Date = Date()) -> String{
        var seconds = Int(abs(self.secondsSince(date)))
        let days = Int(seconds / 86400)
        ...
        if days > 0 {
            ...
            return "\(hours)hour \(mins)min \(seconds)sec"
        }
    }
    
    static func timeIntervalString() -> String {
